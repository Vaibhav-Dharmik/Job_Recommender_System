...
from DBOperations import updateCluster
...
class KMeans:
    ...
    def __init__(self, filename, knums):
        ...
        self._knums = knums
        self._dimension = 0
        """self._samples := [(seqx, x1, x2, ..., xn), (seqy, y1, y2, ..., yn), ..., (seqz, z1, z2, ..., zn)]"""
        self._samples= []
    ...
    def _normalize(self):
        ...
        for t in range(len(new_samples)):
    ...
    def flush(self, filename):
        ...
        print("clusters")
        print(self._clusters)
        for c in range(self._knums):
            foutput.write("Group %d" % c)
            for e in self._clusters[c][1:]:
                #foutput.write("%s" %  repr(e))
                foutput.write("%s" %  str(e[0]))
                updateCluster(int(e[0]),int(c))
            foutput.write("\n\n\n")
        print("Done.")
        foutput.close()
    ...
    def _reconstruct(self, idx):
        ...
        for i in range(len(self._clusters[idx])):
